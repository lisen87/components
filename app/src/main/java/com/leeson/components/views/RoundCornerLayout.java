...
public class RoundCornerLayout extends RelativeLayout {
        ...
        public RoundCornerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            mRoundAsCircle = ta.getBoolean(R.styleable.RoundCornerLayout_round_as_circle, false);
            mStrokeColor = ta.getColor(R.styleable.RoundCornerLayout_stroke_color, Color.WHITE);
            mStrokeWidth = ta.getDimensionPixelSize(R.styleable.RoundCornerLayout_stroke_width, 0);
            int roundCorner = ta.getDimensionPixelSize(R.styleable.RoundCornerLayout_round_corner, 0);
            int roundCornerTopLeft = ta.getDimensionPixelSize(
                    R.styleable.RoundCornerLayout_round_corner_top_left, roundCorner);
            int roundCornerTopRight = ta.getDimensionPixelSize(
                    R.styleable.RoundCornerLayout_round_corner_top_right, roundCorner);
            int roundCornerBottomLeft = ta.getDimensionPixelSize(
                    R.styleable.RoundCornerLayout_round_corner_bottom_left, roundCorner);
            int roundCornerBottomRight = ta.getDimensionPixelSize(
                    R.styleable.RoundCornerLayout_round_corner_bottom_right, roundCorner);
